...
class ConsentControllerIntegrationTests {
  ...
  void shouldCreateConsentSuccessfullyAndLogIt() {
    ConsentRequestCreateDTO request =
        new ConsentRequestCreateDTO(
            CPF_VALIDO_1, ConsentStatus.ACTIVE, LocalDateTime.now().plusDays(30), null);

    ConsentResponseDTO createdConsent =
        webTestClient
            .post()
            .uri(API_URL)
    ...
    StepVerifier.create(consentLogRepository.findAll())
        .expectNextMatches(
            log ->
                log.getConsentId().equals(createdConsent.id())
  ...
  void shouldReturnBadRequestForInvalidCpf() {
    ConsentRequestCreateDTO request =
        new ConsentRequestCreateDTO(
            "invalid-cpf", ConsentStatus.ACTIVE, LocalDateTime.now().plusDays(30), INFO_TESTE_1);
  ...
  void shouldListAllConsents() {
    ...
    ParameterizedTypeReference<PageDTO<ConsentResponseDTO>> pageDtoType =
  ...
  void shouldUpdateConsent() {
    ...
    boolean exists =
        consentLogRepository
            ...
            .any(
                consentLog ->
                    consentLog.getConsentId().equals(beforeUpdatedInDb.getId())
  ...
  @Test
  @DisplayName(
      "PUT /consents/{id} - Deve retornar 404 Not Found ao tentar atualizar um consentimento inexistente")
  ...
  void shouldReturnBadRequestWhenUpdatingWithInvalidCpf() {
    ...
    ConsentRequestUpdateDTO request =
        new ConsentRequestUpdateDTO("123456789", ConsentStatus.REVOKED, null, null);
  ...
  void shouldDeleteConsent() {
    ...
    boolean exists =
        consentLogRepository
            ...
            .any(
                consentLog ->
                    consentLog.getConsentId().equals(savedConsent.getId())
  ...
  @Test
  @DisplayName(
      "DELETE /consents/{id} - Deve retornar 404 Not Found ao tentar deletar consentimento inexistente")
