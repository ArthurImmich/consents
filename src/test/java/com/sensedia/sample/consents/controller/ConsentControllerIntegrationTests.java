...
class ConsentControllerIntegrationTests {
    ...
    void shouldCreateConsentSuccessfullyAndLogIt() {
        ...
        ConsentResponseDTO createdConsent = webTestClient
                .post()
                .uri(API_URL)
        ...
        StepVerifier.create(consentLogRepository.findAll())
                .expectNextMatches(
                        log -> log.getConsentId().equals(createdConsent.id())
    ...
    void shouldListAllConsents() {
        Consent c1 = Consent.builder()
                .id(UUID.randomUUID())
                .cpf(CPF_VALIDO_1)
                .status(ConsentStatus.ACTIVE)
                .build();
        Consent c2 = Consent.builder()
                .id(UUID.randomUUID())
                .cpf(CPF_VALIDO_2)
                .status(ConsentStatus.REVOKED)
                .build();
        Consent c3 = Consent.builder()
                .id(UUID.randomUUID())
                .cpf(CPF_VALIDO_3)
                .status(ConsentStatus.ACTIVE)
                .build();
    ...
    void shouldUpdateConsent() {
        ...
        boolean exists = consentLogRepository
                .findAll()
                .any(
                        consentLog -> consentLog.getConsentId().equals(beforeUpdatedInDb.getId())
    ...
    void shouldDeleteConsent() {
        ...
        boolean exists = consentLogRepository
                .findAll()
                .any(
                        consentLog -> consentLog.getConsentId().equals(savedConsent.getId())
