...
public class GlobalExceptionHandler {
  ...
  public Mono<ErrorDTO> handleValidationException(
      WebExchangeBindException ex, ServerWebExchange exchange) {
    log.warn(
        "Validation failed for request [{}]: {}", exchange.getRequest().getPath(), ex.getMessage());

    List<ErrorDTO.ValidationError> validationErrors =
  ...
  public Mono<ErrorDTO> handleResourceNotFoundException(
  ...
  @ResponseStatus(HttpStatus.BAD_REQUEST)
  public Mono<ErrorDTO> handleIllegalArgumentException(
      IllegalArgumentException ex, ServerWebExchange exchange) {
    log.warn(
        "Illegal argument for request [{}]: {}", exchange.getRequest().getPath(), ex.getMessage());
  ...
  public Mono<ErrorDTO> handleGenericException(Exception ex, ServerWebExchange exchange) {
    log.error(
        "An unexpected error occurred processing request [{}]: {}",
        exchange.getRequest().getPath(),
        ex.getMessage(),
        ex);
